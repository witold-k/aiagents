...
impl<'a> LoadFilePart<'a> {
    pub fn from_json(
        ...
        let start = payload.get("start").or_else(|| payload.get("pos")).and_then(|v| v.as_i64()).unwrap_or(0) as i32 - 1;
        let count = payload.get("count").or_else(|| payload.get("len")).and_then(|v| v.as_i64()).unwrap_or(0) as i32;
