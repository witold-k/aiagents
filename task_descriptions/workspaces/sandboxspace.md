...
```
* **Rule:** Place the entire file unmodified/rewritten inside the raw block format.
...
## 1.4. Raw Block Enforcement (`RAW_TEXT_BEGIN>>` / `<<RAW_TEXT_END`)
* **Rule 1:** Fields using raw blocks MUST use the exact syntax: `RAW_TEXT_BEGIN>>\n[CONTENT]\n<<RAW_TEXT_END`.
* **Rule 2:** The closing tag `<<RAW_TEXT_END` must stand on its own line.
* **Rule 3:** Never escape, quote, or wrap raw block text inside a JSON string.
